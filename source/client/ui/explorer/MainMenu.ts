...
export default class MainMenu extends DocumentView
{
    ...
    protected render()
    {
        ...
        return html`
            ...
            ${fullscreenButtonVisible && ui.isShowing(EUIElements.fullscreen) ? html`<ff-button id="fullscreen-btn" aria-pressed=${fullscreenActive} icon="expand" title=${language.getLocalizedString("Fullscreen")}
                ?selected=${fullscreenActive} @click=${this.onToggleFullscreen}></ff-button>` : null}
            ${toolButtonVisible && ui.isShowing(EUIElements.tools) ? html`<ff-button id="tools-btn" icon="tools" title=${language.getLocalizedString("Tools and Settings")}
